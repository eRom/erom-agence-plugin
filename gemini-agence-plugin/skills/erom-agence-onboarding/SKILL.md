---
name: erom-agence-onboarding
description: "Bootstrap projet eRom - GitHub repo privé, Linear Project EAT, Slack canal privé, structure locale."
---

> [!IMPORTANT]
> **Optimisation des ressources (modèle léger)** : Si tu es l'agent principal, tu DOIS déléguer l'intégralité de l'exécution de cette skill au sous-agent **@erom-builder** via l'outil `start_subagent`. Ne l'exécute pas toi-même.
...
Remplis-le avec les IDs de ton agence (team Linear, statuts, labels, projet Handoffs, channel Slack, comptes agents), puis relance /erom-agence-onboarding.
```
...
- **Non, mais un bloc `## ... Onboarding (eRom)` traîne dans `GEMINI.md`** (projet onboardé avant la migration) → c'est un cas de **migration** : récupère les IDs de ce bloc, ils serviront à écrire `_memory_/ONBOARD.md` en étape 4 (le bloc legacy sera retiré de `GEMINI.md` à ce moment-là).
- **Ni l'un ni l'autre** → on part en mode discovery pur (lookups par nom uniquement).

C'est `_memory_/ONBOARD.md` qui sert d'ancre d'idempotence durable (fichier dédié, neutre, agnostique), pas `GEMINI.md`.
...
Enregistre le projet dans le **vault gerber** pour qu'il soit cloné (cron) puis indexé (FileSearchStore Gemini). Le vault est un repo Git dont une copie locale vit dans `~/.config/gerber-vault` ; on opère directement sur ce clone (édition + commit + push), pas via l'API GitHub.
...
5. **Commit + push** - n'ajoute **que** `sources.yml` (le clone contient du bruit Obsidian non commité, ne jamais faire `git add .`) :
   ...
   Ajouté via la skill erom-agence-onboarding."
...
**Migration depuis `GEMINI.md` (legacy).** Si `_memory_/ONBOARD.md` n'existait pas mais qu'un bloc `## ... Onboarding (eRom)` traîne dans `GEMINI.md` (repéré en étape 1bis) :
1. Écris `_memory_/ONBOARD.md` avec les valeurs (fraîches si lookups OK, sinon celles du bloc legacy).
2. **Retire** la section `## ... Onboarding (eRom)` de `GEMINI.md` (uniquement cette section ; ne touche à AUCUNE autre section custom que Romain aurait ajoutée).
...
- `_memory_/ONBOARD.md` est (ré)écrit avec les valeurs courantes (contenu identique si rien n'a bougé). Un éventuel bloc legacy encore présent dans `GEMINI.md` est retiré (migration one-shot).
