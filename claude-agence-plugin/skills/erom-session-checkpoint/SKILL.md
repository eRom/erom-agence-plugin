...
Appelle le subagent `erom-scribe` (tool Agent, `subagent_type: erom-scribe`). Passe-lui dans le prompt : le handoff-pointeurs, le path du transcript (étape 5), le path du script `<base dir de ce skill>/scripts/condense-transcript.ts`, le dossier `_sessions_/` cible, le timestamp `YYYYMMDD-HH_MM`, le slug. Il écrit le fichier et te renvoie `path` + `next_step`.
