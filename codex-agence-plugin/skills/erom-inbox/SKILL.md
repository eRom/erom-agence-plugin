...
> [!IMPORTANT]
> Codex ne lance pas automatiquement cette skill dans un sous-agent depuis le frontmatter Claude. Si tu es l'agent principal, délègue l'exécution complète au sous-agent `erom-reader`, puis synthétise uniquement son résultat final.
...
Lance /erom-agence-onboarding pour (re)générer _memory_/ONBOARD.md.
```
